...
class Itu8itemSpider(scrapy.spiders.Spider):
    name = "itu8item"
    ...
    def see_item(self, response):
        ...
        #     f.write(str)
        # with open('itu8item', 'wb') as f:
        #     f.write(str)
        # return
        img = ''
        img = Selector(text=str).xpath(
                '//div[contains(@class, "main-image")]/p[contains(@align, "center")]/a/img/@src').extract_first()


        self.item_org_imgs.append(img)

        list2 = Selector(text=str).xpath('//div[contains(@class, "pagenavi")]/a')
        next_page_url = ''
        for index2, li2 in enumerate(list2):
            nav_text = li2.xpath('span/text()').extract_first()
            if nav_text == '下一页':
                next_page_url = li2.xpath('@href').extract_first()

        if (not next_page_url) or (len(next_page_url) == 0):
            # print self.item_org_imgs
            self.iteminfo['content'] = ''
            self.iteminfo['item_org_imgs'] = self.item_org_imgs
            self.iteminfo['item_small_imgs'] = ''
            self.iteminfo['data_time'] = ''
            self.item_update()
    ...
    def item_find(self):
        self.iteminfo={}
        one = self.collection.find_one({"data_source":"http://www.itu8.cc","item_org_imgs":{"$exists":False},"url":{"$exists":True}})
